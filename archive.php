            ...
            <header class="mb-8 border-b border-gray-200 pb-6">
                <?php if (have_posts()) : ?>
                    <h1 class="text-3xl lg:text-4xl font-bold text-gray-900 mb-4">
                        <?php
                        if (is_category()) {
                            printf(__('Category: %s', 'faradalam'), single_cat_title('', false));
                        } elseif (is_tag()) {
                            printf(__('Tag: %s', 'faradalam'), single_tag_title('', false));
                        } elseif (is_author()) {
                            printf(__('Author: %s', 'faradalam'), get_the_author());
                        } elseif (is_date()) {
                            if (is_year()) {
                                printf(__('Year: %s', 'faradalam'), get_the_date('Y'));
                            } elseif (is_month()) {
                                printf(__('Month: %s', 'faradalam'), get_the_date('F Y'));
                            } elseif (is_day()) {
                                printf(__('Day: %s', 'faradalam'), get_the_date('F j, Y'));
                            }
                        } else {
                ...
                <?php else : ?>
                    <div class="col-span-full text-center py-12">
                        <h2 class="text-2xl font-bold text-gray-600 mb-4">
                            <?php _e('No posts found', 'faradalam'); ?>
                        </h2>
                        <p class="text-gray-500">
                            <?php _e('Sorry, but nothing matched your archive criteria.', 'faradalam'); ?>
                        </p>
                    </div>
                <?php endif; ?>
            </div>
            ...
            <?php if (have_posts()) : ?>
                ...
                <!-- <div class="flex justify-center items-center space-x-2 mb-8">
                    ...
                    the_posts_pagination(array(
                        'mid_size'  => 2,
                        'prev_text' => __('&larr; Previous', 'faradalam'),
